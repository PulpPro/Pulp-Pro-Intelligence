const ColourScanner = (() => {
    ...
    function detectStage(r, g, b) {
        const input = { r, g, b };
        const distances = COLOUR_STAGES.map(stage => ({ stage, dist: colourDistance(input, hexToRgb(stage.hex)) }));
    ...
    function init() {
        ...
        showView('cs-scanner'); updateScanButton(); renderMultiScans();
        loadAIModel();
    }
    ...
    async function doScan() {
        ...
        if (!result) {
            ...
            if (rgb) result = detectStage(rgb.r, rgb.g, rgb.b);
            else result = { type: 'single', stage: COLOUR_STAGES[Math.floor(Math.random()*COLOUR_STAGES.length)] };
        }
    ...
    function getBatchResult() {
        ...
        let batchResult;
        if(diff<0.15||!above) batchResult={type:'single',stage:lower||COLOUR_STAGES[COLOUR_STAGES.length-1]};
        else batchResult={type:'range',lower:lower,upper:above};
    ...
    function updateNotePreview(val) {
        ...
        if(preview&&previewText){if(val.trim().length>0){preview.classList.remove('hidden');previewText.innerText=val;}else preview.classList.add('hidden');}
    }
    ...
    async function shareReport() {
        ...
        const canvas=await generateReportCanvas({type:'single',label,shelf,status,colour,note,photo,timestamp:getNow()});
    ...
    async function shareBatchReport() {
        ...
        const note=getNote('csBatchNoteInput'), count=document.getElementById('csBatchCount')?.innerText||'';
    ...
    async function generateReportCanvas({type,label,shelf,status,colour,note,photo,timestamp}) {
        const W=1080, canvas=document.createElement('canvas'), ctx=canvas.getContext('2d');
        let H=900; if(note) H+=80;
        canvas.width=W; canvas.height=H;
        ctx.fillStyle='#0a0a0a'; ctx.fillRect(0,0,W,H);
        if(photo){
            const img=await loadImage(photo);
            ctx.save(); roundRect(ctx,40,40,W-80,400,40); ctx.clip();
            ctx.drawImage(img,40,40,W-80,400); ctx.restore();
            const grad=ctx.createLinearGradient(0,300,0,440);
            grad.addColorStop(0,'rgba(10,10,10,0)'); grad.addColorStop(1,'rgba(10,10,10,0.95)');
            ctx.fillStyle=grad; ctx.fillRect(40,40,W-80,400);
        } else {
            ctx.fillStyle='rgba(22,22,24,0.98)'; roundRect(ctx,40,40,W-80,400,40); ctx.fill();
        }
        ctx.fillStyle=colour; roundRect(ctx,60,460,100,100,20); ctx.fill();
        ctx.fillStyle='#ffffff'; ctx.font='bold 52px -apple-system, sans-serif'; ctx.fillText(label,180,520);
        ctx.fillStyle='rgba(255,255,255,0.08)'; roundRect(ctx,60,580,W-120,12,6); ctx.fill();
        const rows=[['Shelf Life',shelf],['Status',status],['Date',timestamp]];
        let y=620;
        rows.forEach(([key,val])=>{
            ctx.fillStyle='rgba(255,255,255,0.35)'; ctx.font='600 28px -apple-system, sans-serif'; ctx.fillText(key.toUpperCase(),60,y);
            ctx.fillStyle='#ffffff'; ctx.font='bold 28px -apple-system, sans-serif'; ctx.fillText(val,400,y); y+=50;
        });
        if(note){
            ctx.fillStyle='rgba(166,226,46,0.1)'; roundRect(ctx,60,y+10,W-120,60,16); ctx.fill();
            ctx.fillStyle='#a6e22e'; ctx.font='600 24px -apple-system, sans-serif'; ctx.fillText('NOTE: '+note,80,y+48); y+=80;
        }
        ctx.fillStyle='rgba(255,255,255,0.06)'; ctx.fillRect(0,H-80,W,80);
        ctx.fillStyle='#a6e22e'; ctx.font='bold 30px -apple-system, sans-serif'; ctx.fillText('PULP PRO',60,H-38);
        ctx.fillStyle='rgba(255,255,255,0.3)'; ctx.font='400 24px -apple-system, sans-serif';
        ctx.fillText('pulp-pro-intelligence.pulpprobrain.workers.dev',W-720,H-38);
        return canvas;
    }

    async function generateBatchReportCanvas({label,shelf,status,colour,note,count,scans,photos,timestamp}) {
        const W=1080, photoH=220;
        let H=200+(scans.length*(photoH+120))+300; if(note) H+=80;
        const canvas=document.createElement('canvas'); canvas.width=W; canvas.height=H;
        const ctx=canvas.getContext('2d');
        ctx.fillStyle='#0a0a0a'; ctx.fillRect(0,0,W,H);
        ctx.fillStyle='#a6e22e'; ctx.font='bold 36px -apple-system, sans-serif'; ctx.fillText('BATCH COLOUR SCAN',60,70);
        ctx.fillStyle='rgba(255,255,255,0.3)'; ctx.font='400 26px -apple-system, sans-serif'; ctx.fillText(timestamp,60,110);
        let y=150;
        for(let i=0;i<scans.length;i++){
            const scan=scans[i], photo=photos[i], c=getResultColour(scan), lbl=getResultLabel(scan), sh=getResultShelfLife(scan);
            ctx.fillStyle='rgba(22,22,24,0.98)'; roundRect(ctx,40,y,W-80,photoH+100,30); ctx.fill();
            if(photo){
                const img=await loadImage(photo);
                ctx.save(); roundRect(ctx,40,y,W-80,photoH,30); ctx.clip();
                ctx.drawImage(img,40,y,W-80,photoH); ctx.restore();
                const grad=ctx.createLinearGradient(0,y+photoH-80,0,y+photoH);
                grad.addColorStop(0,'rgba(22,22,24,0)'); grad.addColorStop(1,'rgba(22,22,24,0.95)');
                ctx.fillStyle=grad; ctx.fillRect(40,y,W-80,photoH);
            }
            ctx.fillStyle=c; roundRect(ctx,60,y+photoH+15,70,70,14); ctx.fill();
            ctx.fillStyle='#ffffff'; ctx.font='bold 36px -apple-system, sans-serif'; ctx.fillText(`Box ${i+1} — ${lbl}`,150,y+photoH+50);
            ctx.fillStyle='rgba(255,255,255,0.4)'; ctx.font='400 26px -apple-system, sans-serif'; ctx.fillText(`Shelf Life: ${sh}`,150,y+photoH+85);
            y+=photoH+120;
        }
        ctx.fillStyle='rgba(166,226,46,0.2)'; ctx.fillRect(40,y,W-80,2); y+=20;
        ctx.fillStyle='#a6e22e'; ctx.font='bold 28px -apple-system, sans-serif'; ctx.fillText('BATCH AVERAGE',60,y+40);
        ctx.fillStyle=colour; roundRect(ctx,60,y+60,80,80,16); ctx.fill();
        ctx.fillStyle='#ffffff'; ctx.font='bold 44px -apple-system, sans-serif'; ctx.fillText(label,160,y+110);
        const batchRows=[['Boxes Scanned',count],['Shelf Life',shelf],['Status',status]];
        let by=y+170;
        batchRows.forEach(([key,val])=>{
            ctx.fillStyle='rgba(255,255,255,0.35)'; ctx.font='600 26px -apple-system, sans-serif'; ctx.fillText(key.toUpperCase(),60,by);
            ctx.fillStyle='#ffffff'; ctx.font='bold 26px -apple-system, sans-serif'; ctx.fillText(val,380,by); by+=46;
        });
        if(note){
            ctx.fillStyle='rgba(166,226,46,0.1)'; roundRect(ctx,60,by+10,W-120,60,16); ctx.fill();
            ctx.fillStyle='#a6e22e'; ctx.font='600 24px -apple-system, sans-serif'; ctx.fillText('NOTE: '+note,80,by+48); by+=80;
        }
        ctx.fillStyle='rgba(255,255,255,0.06)'; ctx.fillRect(0,H-80,W,80);
        ctx.fillStyle='#a6e22e'; ctx.font='bold 30px -apple-system, sans-serif'; ctx.fillText('PULP PRO',60,H-38);
        ctx.fillStyle='rgba(255,255,255,0.3)'; ctx.font='400 24px -apple-system, sans-serif';
    ...
    function resetScanner(){
        ...
        const batchBtn=document.getElementById('csBatchBtn'); if(batchBtn) batchBtn.style.display='none';
